...
import xiaojinzi.android.adapter.MyDrawerListener;
...
import xiaojinzi.android.view.popupMenu.SmartMenu;
...
public class MainActivity extends BaseFragmentActivity implements SmartMenu.OnMenuCloseListener, View.OnClickListener {
    ...
    public void handle(View v, Integer position) {
        ...
        if (position == 3) {
            ...
            if (MyApp.skin.getEnvironment() == Skin.DAY_ENVIRONMENT) {
                //当前是日间模式,要切换成夜晚模式
                SPUtil.put(this, Constant.SP.homeFragment.themeMode, 1);
                MyApp.skin.loadNightStyle(this);
                EBus.postEvent(Constant.CHANGEMODE_FLAG, 1);
            } else {
                //当前是夜晚模式,要切换成日间模式
                SPUtil.put(this, Constant.SP.homeFragment.themeMode, 0);
                MyApp.skin.loadDayStyle(this);
