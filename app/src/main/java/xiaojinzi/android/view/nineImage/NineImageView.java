package xiaojinzi.android.view.nineImage;
...
import xiaojinzi.android.view.common.RectEntity;
...
public class NineImageView extends ViewGroup {
    ...
    private void computeViewsLocation() {
        ...
        //得到每行的高度
        int eachRowHeight = mHeight / rows;
        //得到每个图片显示的宽度
        int eachViewWidth = mWidth / column;
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < column; j++) {
                ...
                r.leftX = j * eachViewWidth;
                r.leftY = i * eachRowHeight;
                r.rightX = (j + 1) * eachViewWidth;
                r.rightY = (i + 1) * eachRowHeight;
        ...
        for (int i = 0; i < childCount % column; i++) {
            RectEntity r = new RectEntity();
            r.leftX = i * eachViewWidth;
            r.leftY = (rows) * eachRowHeight;
            r.rightX = (i + 1) * eachViewWidth;
            r.rightY = (rows + 1) * eachRowHeight;
            rectEntityList.add(r);
        }
